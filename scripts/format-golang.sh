...
goimports -v -w -e -local github.com/mecha-ci main.go
goimports -v -w -e -local github.com/mecha-ci internal/

echo "formatting with gci.."
echo "${GOFILES}" | \
xargs -I {} sh -c 'gci write --skip-generated -s standard -s default -s "Prefix(github.com/mecha-ci)" {}'
